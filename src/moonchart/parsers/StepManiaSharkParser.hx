...
typedef SSCStep = Array<StepManiaNote>;
typedef SSCMeasure = Array<SSCStep>;

typedef StepManiaSharkNotes =
{
	var dance:StepManiaDance;
	var diff:String;
    var desc:String;
	var notes:Array<SSCMeasure>;
}
...
typedef SSCFormat = 
{
	...
	NOTES:Map<String, StepManiaSharkNotes>,
...
class MSDFIle
{
    ...
    public function new(fileContents:String)parseContents(fileContents);
    

    public function parseContents(content:String){ // Based on the MSD parser found in Stepmania. Could probably be rewritten to take advantage of Haxe systems and reduce complexity
		...
		while(idx < len){
            ...
            if(char == '#' || char == ':' || char == ';'){
                if(char == ';')currentlyReadingValue = false;
                idx++;
                continue;
            }
...
class StepManiaSharkParser extends BasicParser<SSCFormat> {
    ...
    function readNoteData(mapData:StepManiaSharkNotes, noteData:String){
        var measures:Array<String> = noteData.split(",");
        for(data in measures){
			var measure:SSCMeasure = [];
            var noteRows = data.split("\n");
            for(row in noteRows){
				var step:SSCStep = row.replace("\n", "").trim().split("");
                if(step.length > 0)measure.push(step);
            }
	...
	override function parse(string:String):SSCFormat
    {
		...
		var currentMapData:StepManiaSharkNotes = {
			desc: "",
                    ...
                    switch(title){
                        ...
                        case 'LABELS':
                            ...
                            for(label_data in data){
								...
								if (workable_data.length == 0)continue;
                        ...
                        default:
                            if(Reflect.hasField(formatted, title))
                                ...
                                formatted.EXTRA_PARAMS.set(title, value);
                    }
                case MAP_INFO:
                    switch(title) {
                        case 'DIFFICULTY':
